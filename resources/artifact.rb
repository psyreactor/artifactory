...
action_class do
  ...
  def download_url
    ...
    download_url = "#{new_resource.protocol}://#{auth}@#{new_resource.url}"
    unless phase.nil?
      download_url += "/#{new_resource.phase}"
    end
    unless context.nil?
      download_url += "/#{new_resource.context}"
    end
...
action :create do
  file "#{new_resource.path}.#{filname}" do
    action :delete
